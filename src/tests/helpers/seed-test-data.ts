 ...
 * 
 * Seeds the test database with communities, users, events, and circles for testing.
 */
...
export async function seedTestData(): Promise<void> {
  try {
    ...
    // Create some subscriptions for testing subscription gating
    ...
    await prisma.subscription.create({
      data: {
        userId: createdUsers[5].id, // Regular member in Bonn
        stripeCustomerId: 'cus_mock_member',
        stripeSubscriptionId: 'sub_mock_member',
        stripePriceId: 'price_mock_basic',
        stripeCurrentPeriodEnd: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000),
        status: 'active'
      }
